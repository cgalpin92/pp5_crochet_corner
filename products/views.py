...
from .models import (Product, YarnCategory, ToolCategory,
                     YarnBrand, ToolBrand, ProductCategory)
...
def all_products(request):
    ...
    if request.GET:
        ...
        if 'product_category' in request.GET:
            ...
            products = products.\
                filter(product_category__name__in=product_categories)
            product_categories = ProductCategory.objects.\
                filter(name__in=product_categories)

        if 'category' in request.GET:
            categories = request.GET['category'].split(',')
            if request.GET['category'] in ['4_ply', 'aran', 'dk']:
                products = products.\
                    filter(yarn_category__yarn_category_name__in=categories)
                categories == YarnCategory.objects.\
                    filter(yarn_category_name__in=categories)

            elif request.GET['category'] in ['crochet_hooks_and_needles',
                                             'counters_and_markers',
                                             'yarn_bowls', 'other']:
                products = products.\
                    filter(tool_category__tool_category_name__in=categories)
                categories = ToolCategory.objects.\
                    filter(tool_category_name__in=categories)

        if 'brand' in request.GET:
            brands = request.GET['brand'].split(',')
            if request.GET['brand'] in ['creative_cotton', 'crochet_society',
                                        'scheepjes_chunky_monkey',
                                        'west_yorkshire_spinners_signature']:
                products = products.filter(yarn_brand__name__in=brands)
                brands == YarnBrand.objects.filter(name__in=brands)

            elif request.GET['brand'] in ['bella_coco', 'clover_armour',
                                          'crochet_society', 'knit_pro',
                                          'pony', 'sirdar']:
        ...
        if 'q' in request.GET:
            ...
            if not query:
                messages.error(request,
                               "You didnt enter anything in the search bar!")
...
def add_product(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Failed to add product. Please check that'
